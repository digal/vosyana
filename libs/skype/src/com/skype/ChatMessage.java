...
public final class ChatMessage extends SkypeObject {
	...
	public enum Type {
		 ...
		 * UNKNOWN - other.
		 */
        SETTOPIC, SAID, ADDEDMEMBERS, SAWMEMBERS, CREATEDCHATWITH, LEFT, UNKNOWN, EMOTED, KICKED, KICKBANNED
    }
     ...
     * @throws SkypeException when connection haselt gone bad.
     */
    ...
    public LeaveReason getLeaveReason() throws SkypeException {
        String reason = getProperty("LEAVEREASON"); 
        if ((reason != null) && (reason.length() > 0)) {
            try {
                return LeaveReason.valueOf(reason);
            } catch (IllegalArgumentException e) {
                System.err.println("Unknown LEAVEREASON: "+reason);
                e.printStackTrace();
                return LeaveReason.UNSUBSCRIBE;
            }
        } else return null;
    }
     ...
     * @param content  the string that is to be this chat message's content
     * @see #getContent
